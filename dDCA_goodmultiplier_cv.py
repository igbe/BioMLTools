...
class deterministicDCA(BaseEstimator,ClassifierMixin):
    ...
    def fit(self,antigen_plus_siganls,issignalOrderedCorrectly = False):
        ...
        print "max_csm = ", max(csms), "min_csm = ", min(csms)
        return self
...
if __name__ == '__main__':
    ...
    label = np.array(convert_class(data_class))
    ...
    for train_index, test_index in skf.split(new_data, label):
        ...
        print "length of test data is = ", len(y_test)


        dc = deterministicDCA(anomaly_threshold,num_of_dc_selected = num_of_dc_selected,lifespan = 2,threshold =(5, 15),signal_weights = ([0, 1, 1], [0, 1, -2]))
        ...
        #
        pred,mcav = dc.fit(tot_signal,issignalOrderedCorrectly = True).predict(tot_signal)
        #print pred
        ...
        plt.plot([i for i in range(len(mcav))], mcav)
        plt.grid()
        plt.show()
        '''
    print "np.array(mean_inst_tpr)", np.array(mean_inst_tpr)
    mean_inst_tpr = np.mean(np.array(mean_inst_tpr))
    mean_inst_fpr = np.mean(np.array(mean_inst_fpr))
    mean_roc_fpr = np.mean(np.array(mean_roc_fpr),axis=0)
    mean_roc_tpr = np.mean(np.array(mean_roc_tpr), axis=0)

    print "mean_inst_tpr = " ,mean_inst_tpr
    print "mean_inst_fpr = ", mean_inst_fpr
